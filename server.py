...
class RecommendationRequest(BaseModel):
    ...
    tone: Optional[str] = "Formal"  # "Formal" | "Friendly" | ... | None (문체 업로드 시)
...
{f"- 어조: {req.tone}에 맞게 일관성 정렬(추천 강도는 본문 의미 훼손 없이 표현 수위만 조정)" if req.tone else "- 어조: 업로드된 문체 분석 결과에 따라 작성자의 고유 문체를 그대로 반영하여 자연스럽고 일관된 어조를 유지하세요."}
...
- 응집성: 문단 연결이 매끄러운가?
{f"- 톤: {req.tone}에 일관적인가?" if req.tone else "- 문체: 업로드된 문체 분석 결과에 일관적인가?"}
- 개인정보: 민감 정보 노출이 없는가?
